...
public class CustomGraphSearch implements SearchObject {
	...
	public ArrayList<SearchNode> search(Problem p) {
		...
		while (true) {
			...
			if (getFrontierNodes().isEmpty()) {
				return new ArrayList<SearchNode>();
			}
			if (insertFront == true) { //stack behavior
				// Depth-first search
				SearchNode node = getFrontierNodes().get(0);  // get the latest stacked node
				...
				// add explored node to set
				explored.add(node);
				
				// get children of node
				ArrayList<GridPos> childStates = p.getReachableStatesFrom(node.getState());
				
				for (GridPos g: childStates) {
					SearchNode childNode = new SearchNode(g, node);
					
					// if child has not been added as explored or to-be-processed
				...
				SearchNode node = getFrontierNodes().get(0);  // get last node from queue
				frontier.removeFirst();
