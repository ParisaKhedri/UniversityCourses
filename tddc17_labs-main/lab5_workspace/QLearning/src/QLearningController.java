...
public class QLearningController extends Controller {
	...
	void performAction(int action) {
		...
		case 0: // no engines
			...
			middleEngine.setBursting(false);
			break;
		case 1: // only left engine
			leftEngine.setBursting(true);
			rightEngine.setBursting(false);
			middleEngine.setBursting(false);
			break;
		case 2: // only right engine
			leftEngine.setBursting(false);
			rightEngine.setBursting(true);
			middleEngine.setBursting(false);
			break;
		case 3: // only middle engine
			leftEngine.setBursting(false);
			rightEngine.setBursting(false);
			middleEngine.setBursting(true);
			break;
		case 4: // left and middle engine
			leftEngine.setBursting(true);
			rightEngine.setBursting(false);
			middleEngine.setBursting(true);
			break;
		case 5: // right and middle engine
			leftEngine.setBursting(false);
			rightEngine.setBursting(true);
			middleEngine.setBursting(true);
			break;
	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				double current_reward = StateAndReward.getRewardAngle(angle.getValue(), vx.getValue(), vy.getValue());
				
				double new_Qtable_value = Qtable.get(prev_stateaction) + alpha(Ntable.get(prev_stateaction))*(current_reward + GAMMA_DISCOUNT_FACTOR * 
						getMaxActionQValue(new_state) - Qtable.get(prev_stateaction));
				
				Qtable.put(prev_stateaction, new_Qtable_value);
